...
set1_far = im2double(imread('https://raw.githubusercontent.com/Fletch235/imagesCOMP572/main/cat.jpg')); 		        % "far" picture
set1_near = im2double(imread(['https://raw.githubusercontent.com/Fletch235/imagesCOMP572/main/dog.jpg'])); 	    		% "near" picture
...
x1 = [7.228837209302327e+02 1.011837209302326e+03]; y1 = [2.235417743324718e+02 2.296571920757965e+02];
x2 = [2.059550408719346e+02 4.263160762942779e+02]; y2 = [2.321723433242506e+02 2.221178474114441e+02];
...
cutoff_high = 20.0;   % provide a value for the high-pass filter cutoff frequency (sigma of gaussian).  CHOOSE CAREFULLY!
...
function im12 = hybridImage(im1, im2, cutoff_low, cutoff_high)
    ...
    FA = fftshift(fft2(im1));
    FB = fftshift(fft2(im2));
    GL = fspecial('gaussian', [h, w], cutoff_low);  GL = GL ./ max(GL(:)); % low-pass
    GH = fspecial('gaussian', [h, w], cutoff_high); GH = GH ./ max(GH(:)); %  high-pass
    H = FA .* GL + FB .* (1 - GH);
    im12 = real(ifft2(ifftshift(H)));
    im12 = min(max(im12, 0), 1);
